...
new Sim('аудит · поиск не переспрашивает категорию')
...
new Sim('аудит · ответ на уточнение текстом')
  ...
  .say('лёгкую')
  .expect('карточки')
  .expectRecentNot('не хочу придумывать')
  .print();

new Sim('аудит · подарок не глотает «я не знаю»')
  .say('Что подарить подруге?')
  .tap('до 5 000 ₽')
  .say('Какой у него SPF?')
  .expect('не хочу придумывать')
  .print();

new Sim('аудит · пропуск вопроса в воронке')
  .act('q:buy')
  .tap('уход')
  .say('Не хочу отвечать на вопросы')
  .expect('пропускаю')
  .expect('что тебе точно не нравится')
  .expectNot('можешь просто написать')
  .print();
